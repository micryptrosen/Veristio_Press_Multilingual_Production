...
|---|---|---|---|---|
| B1-ES-GLS-GAP-001 | Book 1 Spanish terminology seed | INTERNAL_REVISIONS_APPLIED_NAMED_REVIEW_PENDING | `BOOK_1_SPANISH_TERMINOLOGY_SEED_V1.md`, `BOOK_1_CHAPTER_1_SPANISH_TERMINOLOGY_PASS_V1.md`, and Draft V2 exist. | Obtain terminology reviewer/operator decision before terminology readiness is claimed. |
...
| B1-ES-GLS-GAP-006 | Title localization decision | INTERNAL_REVISION_PRESERVE_TITLE | Internal terminology pass and Draft V2 preserve `Human Creativity + AI Synthesis` as title treatment. | Reviewer/operator must preserve or authorize a localized title later. |
| B1-ES-GLS-GAP-007 | Nuanced term decisions | RESOLVED_PROVISIONAL_NAMED_REVIEW_PENDING | Revision resolution log records provisional decisions for `source truth`, `authority gate`, `evidence trail`, `teammanship`, and `prompt`. | Reviewer/operator must approve, revise, or defer terms before terminology readiness is claimed. |

## Draft Readiness Impact

Terminology seed, internal terminology pass, revision resolution log, and Draft V2 exist, but reviewer availability is not established. Native-review preparation may continue only under provisional draft-only boundaries until repo truth records named reviewer coverage or an operator-approved alternate.
